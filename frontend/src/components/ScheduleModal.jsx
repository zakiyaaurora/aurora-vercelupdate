...
export default function ScheduleModal({
  ...
  useEffect(() => {
    if (!open || !product?.id || !startDate || !endDate) {
      setRangeAvail(null);
      return;
    }
  ...
  const listedUnits = useMemo(() => {
    ...
    return units.map((u) => ({
      ...u,
      occupancies: (u.occupancies || []).filter((o) =>
        inRange(focusDay, o.start_date, o.end_date)
      ),
    }));
  }, [units, focusDay]);
    ...
    <Modal
      ...
      title={
        <span className="flex items-center gap-2">
          <CalendarDays className="h-5 w-5 text-[#E83E8C]" />
          Jadwal — {product?.name || ""}
        </span>
      }
      footer={
        <Btn
          variant="outline"
          onClick={onClose}
          data-testid="schedule-close-btn"
        >
          Tutup
        </Btn>
      }
    >
      <div data-testid="schedule-modal">
        ...
        {startDate && endDate && (
          <div
            className={cn(
              "rounded-xl border px-4 py-3 mb-4 flex flex-wrap items-center justify-between gap-2",
              st.tone
            )}
            data-testid="schedule-range-summary"
          >
            <p className="text-sm">
        ...
        {!internal && (
          <p
            className="mb-3 text-xs text-[#7A6A75] flex items-center gap-1.5"
            data-testid="schedule-public-notice"
          >
            <Lock className="h-3.5 w-3.5" />
            Tampilan publik — identitas penyewa disamarkan.
          </p>
        )}
        ...
        <div className="grid lg:grid-cols-12 gap-5">
          ...
          <div className="lg:col-span-7">
            <div className="flex items-center justify-between mb-3">
              ...
              <p
                className="font-semibold text-[#1F191E] capitalize"
              ...
              <div className={cn("transition-opacity", loading && "opacity-60")}>
                <div className="grid grid-cols-7 gap-1 mb-1">
                  {DAYS.map((d) => (
                    <div
                      key={d}
                      className="text-center text-[11px] font-semibold text-[#B79BAA] uppercase"
                    >
                      {d}
                    </div>
                  ))}
                  ...
                  {weeks.flat().map((c) => {
                    ...
                    return (
                      <button
                        ...
                        title={
                          c.inMonth
                            ? `${formatDateShort(c.iso)}: ${busy} dari ${total} unit terpakai`
                            : ""
                        }
                        data-testid={
                ...
                <div className="mt-3 flex flex-wrap gap-3 text-[11px] text-[#7A6A75]">
                  ...
                  <Legend
                    cls="border-[#E83E8C] ring-1 ring-[#E83E8C]"
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              {focusDay && (
                <button
                  ...
                  data-testid="schedule-clear-focus"
                >
                            ...
                            {isCuci ? (
                              ...
                              <div className="rounded-lg border border-[#FDE68A] bg-[#FFFBEB] p-3">
                                ...
                                {internal && o.ref_number && (
                                  <p className="mt-1.5 font-mono text-[11px] text-[#7A6A75]">
                                    {o.ref_number}
                                  </p>
                                )}
                              </div>
                              ...
                              <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                                ...
                                <span className="text-[#4A3F47]">
                                  {formatDateShort(o.start_date)} –{" "}
                                  {formatDateShort(o.end_date)}
                                </span>

                                <span
                                  className="font-medium text-[#1F191E]"
                                  data-testid="occupancy-customer"
                                >
                                  {o.customer_name}
                                </span>

                                {o.ref_number && (
                                  <span className="font-mono text-[#7A6A75]">
                                    {o.ref_number}
                                  </span>
                                )}

                                {o.customer_phone && (
                                  <span className="inline-flex items-center gap-1 text-[#7A6A75]">
                                ...
                                {o.is_overdue && (
                                  <span className="rounded-full bg-[#FEF2F2] text-[#B91C1C] border border-[#FECACA] px-2 py-0.5 font-semibold">
                                    Terlambat
                                  </span>
                                )}
                              </div>
