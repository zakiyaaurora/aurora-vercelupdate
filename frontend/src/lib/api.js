...
export const listInventory = () =>
  supabase
    ...
    .select(
      "*, product:products(id,name,product_code,rental_price)"
    )
    .order("created_at", { ascending: false })
    .then(throwOn);
...
export const getBooking = (id) =>
  supabase
    .from("bookings")
    .select(
      "*, customer:customers(*), items:booking_items(*, product:products(id,name,product_code), inventory:inventory_items(id,sku))"
    )
    .eq("id", id)
    .single()
    .then(throwOn);
...
export const getRental = (id) =>
  supabase
    .from("rentals")
    .select(
      "*, customer:customers(*), items:rental_items(*, product:products(id,name,product_code), inventory:inventory_items(id,sku))"
    )
    .eq("id", id)
    .single()
    .then(throwOn);
...
export const getInvoice = (id) =>
  supabase
    .from("invoices")
    .select(
      "*, customer:customers(*), booking:bookings(*, items:booking_items(*, product:products(id,name,product_code))), payments:payments(*)"
    )
    .eq("id", id)
    .single()
    .then(throwOn);
...
export const listPayments = () =>
  supabase
    .from("payments")
    .select(
      "*, customer:customers(id,name), invoice:invoices(id,invoice_number)"
    )
    .order("created_at", { ascending: false })
    .then(throwOn);
...
export const listStockMovements = () =>
  supabase
    ...
    .select(
      "*, product:products(id,name), inventory:inventory_items(id,sku)"
    )
    .order("created_at", { ascending: false })
    .limit(200)
    .then(throwOn);
...
export const dashboardStats = () =>
  supabase.rpc("dashboard_stats").then(throwOn);
